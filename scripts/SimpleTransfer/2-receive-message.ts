...
async function main() {
  ...
  // --- 3. Configuration ---
  // Deployment ID for CCTPHookWrapper. Verify this matches your deployment.
  const hookDeploymentId = "CCTPHookWrapperModule#CCTPHookWrapper";

  // --- 4. Get Deployed Contract (Reading deployed_addresses.json) ---
  console.log(
    `\nGetting deployed CCTPHookWrapper address for ID: ${hookDeploymentId} on chain ${chainId}...`
  );

  let hookContract: CCTPHookWrapper;
  ...
  try {
    ...
    hookContract = (await ethers.getContractAt(
      CCTPHookWrapperArtifact.abi,
      hookContractAddress,
      deployer // Use the deployer/signer for the transaction
    )) as unknown as CCTPHookWrapper;

    console.log("Successfully retrieved CCTPHookWrapper contract instance.");
  } catch (error: unknown) {
    ...
    console.error(
      `1. Ignition deployment for CCTPHookWrapper (ID: ${hookDeploymentId}) on chain ${chainId} has run successfully.`
    );
    console.error(
  ...
  console.log(
    `\nCalling receiveMessage on CCTPHookWrapper (${hookContractAddress})...`
  );
  try {
    ...
    console.log(
      `  Message received successfully. Gas used: ${receipt?.gasUsed.toString()}`
    );
    console.log(
    ...
    let failMessage = "Failed to execute receiveMessage.";
    if (
